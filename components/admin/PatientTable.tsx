...
export default function PatientTable({ data }: PatientTableProps) {
  ...
  const filteredPatients = data.filter(patient => {
    ...
    return (
      ...
      patient.plan.toLowerCase().includes(searchLower)
    );
  });
            ...
            >
              ...
              <div 
                className={`absolute top-0 inset-x-0 h-1.5 ${
                  patient.plan.toLowerCase().includes('premium') 
                    ? 'bg-gradient-to-r from-purple-500 to-pink-500' 
                    : patient.plan.toLowerCase().includes('starter')
                    ? 'bg-pink-500'
                    : 'bg-indigo-500'
                }`}
              />
              ...
              <div>
                ...
                <div className="flex justify-between items-center text-[10px] text-slate-400 mt-2 font-mono">
                  ...
                  <span>Enrolled: {format(new Date(patient.createdAt), 'MMM dd, yyyy')}</span>
                </div>
              </div>
            </div>
          ))}
