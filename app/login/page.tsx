...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${getPublicApiBase()}/doctors/login`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        // Store both token and doctor data
        localStorage.setItem('doctorToken', data.token);
        localStorage.setItem('doctorData', JSON.stringify(data.doctor));
        
        // Redirect to dashboard using the doctor's real ID from database
        router.push(`/doctor/${data.doctor.id}/dashboard`);
      } else {
      ...
      <div className="grid lg:grid-cols-2 min-h-screen">
        ...
        <div className="hidden lg:flex items-center justify-center bg-gradient-to-br from-pink-50 via-white to-purple-50 px-20">
          <motion.div
            ...
            className="max-w-xl"
          >
            <div className="text-7xl text-pink-200 mb-6">“</div>
            ...
            <p className="text-2xl leading-relaxed text-slate-600">
              WombCare helps women take control of PCOD, periods, fertility,
              and overall wellness through expert guidance, tracking, and
              personalized care.
            </p>
            ...
            <div className="mt-10 flex items-center gap-4">
              ...
              <div>
                <p className="font-semibold text-slate-800">WombCare</p>
